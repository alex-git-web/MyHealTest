...
import { ContentDisplayedElementsType, ContentElementsDisplayModeType } from '../../../types';
...
export const KebubMenuBtn = ({
  ...
  return (
    <>
      <button
        className={`"header__kebab-menu-btn kebab-menu-btn ${isKebabMenuBtnClicked ? ' active' : ''}`}
      ...
      {isKebabMenuBtnClicked && (
        <CardKebabMenuPopUp displayMode={displayMode} displayedElementsType={displayedElementsType} />
      )}
    </>
  );
};

export const OpenFilesBtn = ({ filesLenght }: { filesLenght: number }) => {
  return (
    <button className="open-files-btn">
      ...
      <span className="open-files-btn__caption">{filesLenght} Файл(и)</span>
    </button>
  );
};
